 ...
 */
export async function optimizePromptWithAnthropic(
...
Optimiza este prompt para Veo 3.`;
    ...
    if (
      !parsedResponse.optimizedPrompt ||
...
function createFallbackOptimization(
  ...
  if (
    !originalPrompt.toLowerCase().includes("alta calidad") &&
  ...
  if (
    !originalPrompt.toLowerCase().includes("cámara") &&
